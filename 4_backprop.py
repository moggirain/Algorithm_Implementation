...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb  5 20:48:42 2019
...
DATA_PATH = "./" #TODO: if doing development somewhere other than the cycle server, change this to the directory where a7a.train, a7a.dev, and a7a.test are
...
def init_model(args):
    ...
    if args.weights_files:
        ...
        w1 = np.random.rand(args.hidden_dim, NUM_FEATURES)-0.5 #bias included in NUM_FEATURES
        w2 = np.random.rand(1, args.hidden_dim + 1)-0.5 #add bias column
...
def sigmoid(x, deriv=False): #  pay attention to the input and output 
	s=1/ (1+ np.exp(-x))
	if deriv == True:
...
#w = weight, z=input from the network, a = output after the activation function, y_hat is the output of output layer, the y_estimate.

    w1 = model['w1'] #5*124
#    print("w1 ", w1.shape)
    w2 = model['w2'] #1*6
#    print("w2 ", w2.shape)
    z1 = np.dot(w1,X)  #(5*124) (124*1) = 5*1
#    print("z1 ", z1.shape)
    a1 = sigmoid(z1) #6 *1
    a1 = np.append(a1,[[1]],axis = 0) # 加bias 6 *1 
#    print("a1 ",a1.shape)
    z2 = np.dot(w2,a1) # w2=1*6  a1=6*1  z2=1*1
#    print("z2 ",z2.shape)
...
def backprop(model,X,y,z1,a1,z2,y_hat):
    
    w2 = model['w2'] #1*6
    derror = y_hat-y #1
    
    delta2=derror * sigmoid(z2,deriv=True) #1*1
    dw2 = np.dot (a1,delta2)  # (6*1) (1*1) = (6*1)
    delta1 = ((np.dot(delta2,w2)[:,:len(a1)-1]))*(sigmoid(z1,deriv=True)).T# (1*1) (1*(6-1))=(1*5)
    dw1 = np.dot(X,delta1).T # (124*1)(1*5)=(5*124)

    return dw1,dw2

def train_model(model, train_ys, train_xs, dev_ys, dev_xs, args):
    """input the training set; output w1, for model training """
    # training model
    ...
    if args.nodev:
        ...
        for j in range(args.iterations): # in each iteration 
            for i in range(len(train_xs)): # for each training sample
                z1, a1, z2, y_hat = forwardprop(model,train_xs[i])
                #backpropagation 
                dw1, dw2 = backprop(model,train_xs[i],train_ys[i],z1,a1,z2,y_hat) 

                # update the value of w1,w2
                w2-= args.lr * dw2.T
                w1-= args.lr * dw1
    ...
    if not args.nodev: 
            ...
            for lr in [0.01, 0.1, 0.5]:
                for hidden_dim in [5, 10]:
                    #dev_acc = [] # store the dev_set accuracy
                    #train_acc = [] # store the training set accuracy for plotting
                    model = init_model(args) # initalize model after each training 
                    w1 = model['w1']
                    w2 = model['w2']
                    args.lr = lr
                    ...
                    for j in range(args.iterations): # in each iteration 
                        ...
                        train_accuracy = test_accuracy(model,train_ys,train_xs)
                        dev_accuracy = test_accuracy(model,dev_ys,dev_xs)
                        
                        # traverse to find the best accuracy
                        if dev_accuracy > train_accuracy:
                            #best_list = [copy.deepcopy(dev_accuracy), args.iterations, lr, hidden_dim]
                            best_weight['w1'] = w1
                            best_weight['w2'] = w2
...
def test_accuracy(model, test_ys, test_xs):
    accuracy = 0.0
    cor = 0.0
    # accuracy=the number of data are classifid correctly 
    for i in range(len(test_xs)): #  iterate every sample
        y_hat = forwardprop(model,test_xs[i])[3]
        if (y_hat >0.5 and test_ys[i] ==1):
            cor +=1
        if (y_hat <0.5 and test_ys[i] ==0):
            cor +=1
    accuracy = cor / len(test_ys)
